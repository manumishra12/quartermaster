    ...
    <div className="flex flex-wrap gap-1.5 px-5 pb-2" role="group" aria-label="Quick actions">
      {ACTIONS.map(({ icon: Icon, label, prompt }) => (
        <button
          ...
          type="button"
          onClick={() => use(prompt)}
          className="inline-flex cursor-pointer items-center gap-1.5 rounded-full border border-line px-2.5 py-1 text-2xs text-muted transition-colors duration-200 hover:border-accent hover:bg-accent-wash hover:text-ink"
        >
          <Icon />
          {label}
        </button>

...
function admit(requested) {
  ...
  try {
    ...
    /**
     * A second name for the same file, which the root cannot see.
     *
     * Confinement resolves symbolic links and then checks where the path landed. A hard link has
     * nothing to resolve: the name inside the root *is* the file, and `realpath` returns the
     * in-root path, so containment passes while the bytes belong to a file outside it. Verified
     * against a running server - a hard link in the root to a file above it read that file's
     * contents and reported them as an ordinary document.
     *
     * Refused on the link count, which is the only thing a descriptor can say about this: `st_nlink`
     * greater than one means the file has another name somewhere, and nothing here can find out
     * where. That is deliberately blunt and it is the safe direction, because a document with two
     * names is not a thing this server has any reason to read. The one place hard links occur in
     * this repository is npm's binaries under `node_modules`, which is refused by name already.
     */
    if (opened.nlink > 1) {
      return {
        refusal: refuse(
          "several_names",
          `${within} has ${opened.nlink} names on this filesystem, so the file it points at may sit outside this ` +
            "server's root - a hard link resolves to itself and the root check cannot see through it. It was not " +
            "read. Copy the document into the root if it belongs there.",
          { path: within, links: opened.nlink, rule: "one-name-only" },
        ),
      };
    }

...
## The second fixture, where a page cannot be read

`appendix.pdf`, generated by `build_appendix.py`, is the other half: three pages, and the middle one
carries a content stream labelled `LZWDecode`, which `pdfread.py` does not implement. The published
answer is **4 requirements from 2 of the 3 pages**, `complete` false, `pages_not_parsed: [2]`, and
the coverage warning quoted in full.

The four it returns are correct, verbatim and cited, and **nothing in the list of four shows that
two are missing**. That is the whole fixture: an incomplete list presented as the requirements is
worse than no list, because the reader stops looking. The answer does not move with the machine -
`needs-ocr` either way, 2 of 3 either way - which a scanned page could not promise, and
`fixture/README.md` writes out the two requirements nobody gets to read.

## Tests

`tools/documents/test_documents.py`, `unittest`, standard library, wired into `npm run check` as
`npm run documents:test`. 36 tests. The ones that matter:
